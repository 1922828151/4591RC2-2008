/*
 * func-name: ??0CMiniWorldMapUI@@QAE@ABV0@@Z_0
 * func-address: 0x10219cc0
 * callers: 0x1000f5b5
 * callees: none
 */

CMiniWorldMapUI *__thiscall CMiniWorldMapUI::CMiniWorldMapUI(CMiniWorldMapUI *this, const struct CMiniWorldMapUI *a2)
{
  CTYDialog::CTYDialog(this, a2);
  *(_DWORD *)this = &CMiniWorldMapUI::`vftable';
  *((_DWORD *)this + 968) = *((_DWORD *)a2 + 968);
  *((_DWORD *)this + 969) = *((_DWORD *)a2 + 969);
  *((_DWORD *)this + 970) = *((_DWORD *)a2 + 970);
  *((_DWORD *)this + 971) = *((_DWORD *)a2 + 971);
  return this;
}
