/*
 * func-name: ?Initialize@Engine@@QAEXAAVConfigFile@@PAUHWND__@@1PAUHINSTANCE__@@PBDV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@_N4@Z
 * func-address: 0x102c9534
 * callers: none
 * callees: none
 */

// attributes: thunk
int __thiscall Engine::Initialize(
        void *this,
        int a2,
        int a3,
        int a4,
        int a5,
        int a6,
        int a7,
        int a8,
        int a9,
        int a10,
        int a11,
        int a12,
        int a13,
        int a14,
        int a15,
        int a16,
        int a17,
        int a18,
        int a19,
        int a20,
        int a21)
{
  return __imp_?Initialize@Engine@@QAEXAAVConfigFile@@PAUHWND__@@1PAUHINSTANCE__@@PBDV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@_N4@Z(
           this,
           a2,
           a3,
           a4,
           a5,
           a6,
           a7,
           a8,
           a9,
           a10,
           a11,
           a12,
           a13,
           a14,
           a15,
           a16,
           a17,
           a18,
           a19,
           a20,
           a21);
}
