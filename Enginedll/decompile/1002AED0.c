/*
 * func-name: ?resize@?$vector@PAVModel@@V?$allocator@PAVModel@@@std@@@std@@QAEXIPAVModel@@@Z
 * func-address: 0x1002aed0
 * callers: 0x1002eb40
 * callees: 0x10024a40, 0x100a1d10
 */

unsigned int __thiscall std::vector<Model *>::resize(_DWORD *this, unsigned int a2, int a3)
{
  unsigned int v4; // ecx
  ...
  else
  {
    ...
    if ( v4 > (unsigned int)v7 )
      invalid_parameter_noinfo();
    return (unsigned int)std::vector<Model *>::_Insert_n((int)this, (int)this, v7, a2 - v6, &a3);
  }
  return result;
}
