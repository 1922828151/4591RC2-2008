/*
 * func-name: ?SaveModel@Serializer@@QAE_NV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@PAVStaticModel@@@Z
 * func-address: 0x1011b1a0
 * callers: none
 * callees: 0x10010930, 0x100977a0, 0x100e2550, 0x1011abe0, 0x1017aa70, 0x101a2500
 */

char __stdcall Serializer::SaveModel(char a1, int a2, int a3, int a4, int a5, int a6, int a7, int *a8)
{
  ISave *Saver; // esi
  const char *v9; // eax
  ...
  if ( Saver )
  {
    ...
    (*(void (__thiscall **)(ISave *, int, int, int, int, int, int, int, int *))(*(_DWORD *)Saver + 4))(
      Saver,
      v11,
      v12,
      v13,
  ...
  else
  {
    ...
    SeriousWarning("No compatible Save serializer for '%s' model.", v9);
    v20 = -1;
    std::string::~string(&a1);
    return 0;
  }
